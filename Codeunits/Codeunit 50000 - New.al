...
//         //NC>>
//         FileHandling: Codeunit "File Handling";
...
//         //         ERASE(MyFile.Path + MyFile.Name);
//         //     END;
//         RequestText := FileHandling.CreateFileExistCheckRequestBody(MyFile.Path + '/' + MyFile.Name);
//         FileHandling.SetFileExistCheckRequestBody(Content, RequestText);
//         IsExist := FileHandling.CheckIfFileExist(Content, Request, ResonseTxt); //If file exist in the certain path ISExist will be TRUE and gets the file content as base64 into ResponseTxt
//         if not IsExist then
//             exit; //Handle ISExist if needed
//         Clear(TempBlob);
//         TempBlob.CreateOutStream(OutStr);
//         Base64Convert.FromBase64(ResonseTxt, OutStr);
...
//             AddToDocNos(LastDocNo_lCod);
//             IF EDISetup."Delete XML on Success" THEN BEGIN
//                 //Deleting the file in the requested path
//                 RequestText := FileHandling.CreateFileDeletionRequestBody(MyFile.Path + '/' + MyFile.Name);
//                 FileHandling.SetFileDeletionRequestBody(Content, RequestText);
//                 IsDeleted := FileHandling.CheckIfFileDeleted(Content, Request);
//                 //if ISDeleted if necessary
//             END ELSE BEGIN
//                 //Getting Content from source file if source file exist in certain path
//                 RequestText := FileHandling.CreateFileExistCheckRequestBody(MyFile.Path + '/' + MyFile.Name);
//                 FileHandling.SetFileExistCheckRequestBody(Content, RequestText);
//                 IsExist := FileHandling.CheckIfFileExist(Content, Request, ResonseTxt);
//                 //Copying content to destination path from source path if source file exist
//                 if IsExist then begin
//                     RequestText := FileHandling.CreateFileCreationRequestBody(EDISetup."XML Success Folder", MyFile.Name, ResonseTxt);
//                     FileHandling.SetFileCreationRequestBody(Content, RequestText);
//                     ISCreated := FileHandling.CheckIfFileCreated(Content, Request);
//                     //Deleting the source file after copying to destionation path
//                     if ISCreated then begin
//                         RequestText := FileHandling.CreateFileDeletionRequestBody(MyFile.Path + '/' + MyFile.Name);
//                         FileHandling.SetFileDeletionRequestBody(Content, RequestText);
//                         IsDeleted := FileHandling.CheckIfFileDeleted(Content, Request);
//                         //Handle ISDeleted if necessary
//                     end;
//                 end;
//             END;
//             //NC<
...
//             //Copying content to destination path from source path if source file exist
//             if IsExist then begin
//                 RequestText := FileHandling.CreateFileCreationRequestBody(EDISetup."XML Error Folder", MyFile.Name, ResonseTxt);
//                 FileHandling.SetFileCreationRequestBody(Content, RequestText);
//                 ISCreated := FileHandling.CheckIfFileCreated(Content, Request);
//                 //Handle ISCreated if necessary
...
//         RequestText := FileHandling.CreateFileExistCheckRequestBody(MyFile.Path + '/' + MyFile.Name);
//         FileHandling.SetFileExistCheckRequestBody(Content, RequestText);
//         IsExist := FileHandling.CheckIfFileExist(Content, Request, ResonseTxt); //If Directory exist in the certain path ISExist will be TRUE and gets the file content as base64 into ResponseTxt
//         //Ignore Response ResponseTxt as its not necessary in this case
//         IF ISExist THEN
//             EXIT(TRUE)
//         ELSE
//             EXIT(FALSE);
