codeunit 50017 "Label Mgmt NIF"
{
    // 09/26/16: JRR: Added code to save Carton Serial#s in Pkg Line
    ...
    // 
    // 
    // NF2.00:CIS.RAM 05/17/2017 Modifed Function
    // NF2.00:CIS.RAM 10/02/2017 Modifed Function PrintLabelsFromPick
    // SM 02172 Added PK_LINE_NO
    // SM 81022 Added BOX_QTY and LN_GROSS_WT on PrintPackageLabel() and PrintPackageLineLabel()
    // SCT 81822 Added COUNTRY_ORGIN on PrintPackageLabel() and  PrintPackageLineLabel()  Originates from Storage Location on Sales Order Special Fields
    // SCT 102422 Added Order_Date on PrintPackageLabel() and  PrintPackageLineLabel()  Originates from Sales Order Header.
    // CIS.Ram 01/09/2022 Added code to update Master Serial Number to rest of package lines
    ...
    begin
        ...
        IF (LabelHeader."Label Usage"=LabelHeader."Label Usage"::Receive) OR
             (LabelHeader."Label Usage"=LabelHeader."Label Usage"::"Receive Line") THEN
          BEGIN
            GetReceiveStation;
    ...
    begin
        ...
        REPEAT
          ...
          CASE LabelContent."Field Code" OF
            'ITEM_NO' : TempLabelValue."Print Value" := "Receive Line"."No.";
            'LOT_NO'  : TempLabelValue."Print Value" := "Receive Line"."Lot No.";
            'MFG_LOT_NO' : TempLabelValue."Print Value" := "Receive Line"."Mfg. Lot No.";
            'ITEM_DESC' : TempLabelValue."Print Value" := "Receive Line".Description;
            'QTY'       : TempLabelValue."Print Value" := DELCHR(FORMAT(UseQty),'=',',');
        //>>IST 012609 CCL $12797 #12797
        //    'PURCH_ORD_NO' : TempLabelValue."Print Value" := FORMAT(Receive."Purchase Order No.");
            'PURCH_ORD_NO' : TempLabelValue."Print Value" := FORMAT(Receive."Source ID");
        //<<IST 012609 CCL $12797 #12797
            'SYS_DATE'  : TempLabelValue."Print Value" := FORMAT(TODAY);
            'SYS_TIME'  : TempLabelValue."Print Value" := FORMAT(TIME);
            ELSE ERROR('Field Code %1 not supported.',LabelContent."Field Code");
          END;
    ...
    begin
        CLEAR(Item);
        //jrr
        CartonPkgNo := "Package Line"."Package No.";
        CartonPkgLnNo := "Package Line"."Line No.";
        ...
        //>> RTT 07-29-05 if ASN customer, use customer part number
                IF TempItemCrossRef."Cross-Reference No."='' THEN
                  BEGIN
        ...
        //<< RTT 07-29-05 if ASN customer, use customer part number
            ...
            'ECN':
              ...
              END;
            'EXT_DOC_NO': TempLabelValue."Print Value" := FORMAT(Package."External Document No.");
            'FRS_NO':
              BEGIN
                IF NOT SalesLineLoaded THEN
                  LoadSalesLine(Package,"Package Line");
            ...
            'GROUP_CODE':
              ...
              END;
            'ITEM_DESC' : TempLabelValue."Print Value" := "Package Line".Description;
            'ITEM_NO' : TempLabelValue."Print Value" := "Package Line"."No.";
            'KANBAN_NO' :
              BEGIN
                IF NOT SalesLineLoaded THEN
                  LoadSalesLine(Package,"Package Line");
            ...
            'LS_LOC' :
              ...
              END;
            'LOT_NO'  : TempLabelValue."Print Value" := "Package Line"."Lot No.";
            'LOT_CREATION':
              BEGIN
            ...
            'MFG_LOT_NO' :
              BEGIN
                IF "Package Line"."Mfg. Lot No."='' THEN
                  BEGIN
                    IF NOT LotInfoLoaded THEN
                      LoadLotNoInfo(Package,"Package Line");
            ...
            'NET_WT':  //base on qty printing/total qty
              ...
              END;
            'PICK_NO' : TempLabelValue."Print Value" := GetPackagePickNo(Package,"Package Line");
            'PLANT_CODE' :
              BEGIN
                IF NOT SalesLineLoaded THEN
                  LoadSalesLine(Package,"Package Line");
        ...
        ///>>SM 021722 START
             'PK_LINE_NO' :
              BEGIN
                IF NOT SalesLineLoaded THEN
                  LoadSalesLine(Package,"Package Line");
                TempLabelValue."Print Value" := FORMAT(TempSalesLine."Line No.");
              END;
        ...
        ///>>SM 081022 START
             'LN_GROSS_WT' :
              BEGIN
                IF NOT SalesLineLoaded THEN
                  LoadSalesLine(Package,"Package Line");
                  TempLabelValue."Print Value" := FORMAT(TempSalesLine."Line Gross Weight");
              END;
             ...
             'BOX_QTY' :
              BEGIN
                IF NOT SalesLineLoaded THEN
                  LoadSalesLine(Package,"Package Line");
                  TempLabelValue."Print Value" := FORMAT(TempSalesLine."Units per Parcel");
              END;

        ///>>SM 081022 END


        ///>>SCT 081822 START  (((Country of Orgin from Special Fields on sales order)))
             'COUNTRY_ORGIN' :
              BEGIN
                IF NOT SalesLineLoaded THEN
                  LoadSalesLine(Package,"Package Line");
        ...
        ///>>SCT 102422 START  (((Order Date on sales order)))
             'ORDER_DATE' :
              BEGIN
                IF NOT SalesLineLoaded THEN
                  LoadSalesLine(Package,"Package Line");
                  TempLabelValue."Print Value" := FORMAT(TempSalesLine."Order Date");
              END;
        ///>>SCT 102422 END
        ...
        //    'SALES_ORD_NO': TempLabelValue."Print Value" := "Package Line"."Sales Order No.";
            'SALES_ORD_NO': TempLabelValue."Print Value" := "Package Line"."Source ID";
        //<<IST 012609 CCL $12797 #12797
            'SERIAL_NO':
              BEGIN
                LabelContent.TESTFIELD("No. Series");
                IF LabelHeader."Label Type" = LabelHeader."Label Type"::" " THEN BEGIN //CIS.RAM050322
                ...
                //>>CIS.RAM050322
                END ELSE BEGIN
                  LabelHeader.TESTFIELD("Label Constant");
                  TempLabelValue."Print Value" := LabelHeader."Label Constant";

                  IF STRLEN(FORMAT(DATE2DMY(TODAY,2))) < 2 THEN
                    TempLabelValue."Print Value" += '0' + FORMAT(DATE2DMY(TODAY,2))
                  ELSE
                    TempLabelValue."Print Value" += FORMAT(DATE2DMY(TODAY,2));

                  IF STRLEN(FORMAT(DATE2DMY(TODAY,1))) < 2 THEN
                    TempLabelValue."Print Value" += '0' + FORMAT(DATE2DMY(TODAY,1))
                  ELSE
                    TempLabelValue."Print Value" += FORMAT(DATE2DMY(TODAY,1));

                  TempLabelValue."Print Value" += COPYSTR(FORMAT(DATE2DMY(TODAY,3)),3,2);
                  PrintValue := TempLabelValue."Print Value";
                  TempLabelValue."Print Value" += FORMAT(NoSeriesMgt.GetNextNo(LabelContent."No. Series",TODAY,TRUE));
                  TempLabelValue."No. Series" := LabelContent."No. Series";
                END;
                //<<CIS.RAM050322
              END;
            'SHIP_ADDR': TempLabelValue."Print Value" := FORMAT(Package."Ship-to Address");
            'SHIP_ADDR2': TempLabelValue."Print Value" := FORMAT(Package."Ship-to Address 2");
            'SHIP_CITY': TempLabelValue."Print Value" := FORMAT(Package."Ship-to City");
            'SHIP_CITYSTZIP' :  TempLabelValue."Print Value" :=
                 COPYSTR(Package."Ship-to City" + ', ' + Package."Ship-to State" + ' ' + Package."Ship-to ZIP Code",1,50);
            'SHIP_CODE': TempLabelValue."Print Value" := FORMAT(Package."Ship-to Code");
            'SHIP_COUNTRY': TempLabelValue."Print Value" := FORMAT(Package."Ship-to Country Code");
            'SHIP_DATE':
              BEGIN
                IF NOT SalesLineLoaded THEN
                  LoadSalesLine(Package,"Package Line");
              ...
              END;
            'SHIP_NAME': TempLabelValue."Print Value" := FORMAT(Package."Ship-to Name");
            'SHIP_NAME2': TempLabelValue."Print Value" := FORMAT(Package."Ship-to Name 2");
            'SHIP_STATE': TempLabelValue."Print Value" := FORMAT(Package."Ship-to State");
            'SHIP_ZIP': TempLabelValue."Print Value" := FORMAT(Package."Ship-to ZIP Code");
            'SID_NO' :
              BEGIN
                IF NOT SalesHeaderLoaded THEN
                   LoadSalesHeader(Package,"Package Line");
    ...
    begin
        ...
        CartonPkgNo := TempPackageLine."Package No.";          //jrr
        CartonPkgLnNo := TempPackageLine."Line No.";     //jrr
        ...
        //<< RTT 07-29-05 if ASN customer, use customer part number
            ...
            'MODEL_YR' :
              BEGIN
                ...
                TempLabelValue."Print Value" := FORMAT(TempSalesLine."Model Year");
              END;
            'NET_WT': TempLabelValue."Print Value" := FORMAT("Package Line"."Net Weight");
            'PICK_NO' : TempLabelValue."Print Value" := GetPackagePickNo(Package,"Package Line");
            'PLANT_CODE' :
              BEGIN
                IF NOT SalesLineLoaded THEN
            ...
            'PURCH_ORD_NO' :
              BEGIN
                ...
                TempLabelValue."Print Value" := FORMAT(TempLotNoInfo."PO Number");
              END;
            'QTY'       : TempLabelValue."Print Value" := DELCHR(FORMAT("Package Line".Quantity),'=',',');
            'RAN_NO' :
              BEGIN
                IF NOT SalesLineLoaded THEN
                  LoadSalesLine(Package,"Package Line");
        ...
        //    'SALES_ORD_NO': TempLabelValue."Print Value" := Package."Sales Order No.";
            'SALES_ORD_NO': TempLabelValue."Print Value" := Package."Source ID";
        //<<IST 012609 CCL $12797 #12797
            'SERIAL_NO':
              BEGIN
                LabelContent.TESTFIELD("No. Series");
        ...
        ///>>SCT 102422 START  (((  Order Date from Sales order)))
             'ORDER_DATE' :
              BEGIN
                IF NOT SalesLineLoaded THEN
                  LoadSalesLine(Package,"Package Line");
    ...
    begin
        ...
        REPEAT
          ...
          CASE LabelContent."Field Code" OF
            'CONTRACT_DESC' : TempLabelValue."Print Value" := ContractHeader.Description;
            'CONTRACT_NO'  : TempLabelValue."Print Value" := ContractLine."Contract No.";
            'CONTRACT_LOC' : TempLabelValue."Print Value" := ContractLine."Contract Location Code";
            'CROSS_REF_DESC' : TempLabelValue."Print Value" := ItemCrossRef.Description;
            'CROSS_REF_NO' : TempLabelValue."Print Value" := ItemCrossRef."Cross-Reference No.";
            'CUST_BIN' : TempLabelValue."Print Value" := ContractLine."Customer Bin";
            'CUST_BIN_QTY' : TempLabelValue."Print Value" := DELCHR(FORMAT(ContractLine."Reorder Quantity"),'=',',');
            'CUST_BIN_MAX' : TempLabelValue."Print Value" := FORMAT(ContractLine."Max. Quantity");
            'CUST_BIN_MIN' : TempLabelValue."Print Value" := FORMAT(ContractLine."Min. Quantity");
            'CUST_NO' : TempLabelValue."Print Value" := ContractLine."Sales Code";
            'CUST_PO_NO' : TempLabelValue."Print Value" := ContractHeader."External Document No.";
            'ITEM_DESC' : BEGIN
                ContractLine.CALCFIELDS("Item Description");
                TempLabelValue."Print Value" := ContractLine."Item Description";
             END;
            'ITEM_NO' : TempLabelValue."Print Value" := ContractLine."Item No.";
            'REV_NO' :  TempLabelValue."Print Value" := ContractLine."Revision No.";
            'SHIP_ADDR': TempLabelValue."Print Value" := FORMAT(ContractHeader."Ship-to Address");
            ELSE ERROR('Field Code %1 not supported.',LabelContent."Field Code");
          END;
    ...
    var
        ...
        CartonLastSrNo: Text;
    begin
        //MESSAGE('Bar Tender components are not installed');
        ...
        IF UPPERCASE(USERID)='IST' THEN
          Preview := TRUE;
        IF UPPERCASE(USERID) = 'ROSENBERGER' THEN
          Preview := TRUE;
        //IF UPPERCASE(USERID) = 'NIFASTNAV\ROSENBERGER' THEN
        //  Preview := TRUE;

        //IF (TempLabelValue."Label Code"='PK USA_PKG') OR (TempLabelValue."Label Code"='PK_USA') THEN
        //  Preview := TRUE;
        //xx
        ...
        IF CartonFirstSrNo  <> '' THEN BEGIN
          IF CartonLastSrNo = '' THEN
             CartonLastSrNo := CartonFirstSrNo;
          PkgLn_L.INIT;
          IF PkgLn_L.GET(CartonPkgNo, CartonPkgLnNo) THEN BEGIN
            IF MastLbl THEN BEGIN
              ...
              MasterPkgLn_L.SETRANGE(Type,PkgLn_L.Type);
              MasterPkgLn_L.SETFILTER("Line No.",'<>%1',PkgLn_L."Line No.");
              MasterPkgLn_L.SETRANGE("No.",PkgLn_L."No.");
            ...
            END ELSE BEGIN
              IF CartonLastSrNo < CartonFirstSrNo THEN
                CartonLastSrNo := CartonFirstSrNo;
              PkgLn_L."Carton First SrNo." := CartonFirstSrNo;
              PkgLn_L."Carton Last SrNo." :=  CartonLastSrNo;

        //      MasterPkgLn_L.RESET;
        //      MasterPkgLn_L.SETRANGE("Package No.",CartonPkgNo);
        //      IF MasterPkgLn_L.FINDFIRST THEN
        //        IF PkgLn_L."Line No." <> MasterPkgLn_L."Line No." THEN
        //          IF PkgLn_L."Master Label SrNo." = '' THEN BEGIN
        //            PkgLn_L."Master Label SrNo." := MasterPkgLn_L."Master Label SrNo.";
        //            WritePackageSerialNumber(2,CartonFirstSrNo,CartonLastSrNo,PkgLn_L."Master Label SrNo.",LabelHeader.Code);
        //          END;
    ...
    begin
        IF SalesHeaderLoaded THEN
          BEGIN
            CLEAR(SalesHeader);
            CLEAR(SalesHeaderLoaded);
          END;

        IF SalesLineLoaded THEN
          BEGIN
            CLEAR(SalesLine);
            CLEAR(SalesLineLoaded);
          END;

        IF ItemCrossRefLoaded THEN
          BEGIN
            CLEAR(ItemCrossRef);
    ...
    begin
        ...
        //IF NOT SalesLine.GET(SalesLine."Document Type"::Order,PackageLine."Source ID",PackageLine."Order Line No.") THEN    // RAM 05/17/2017
        //  IF NOT SalesLine.GET(SalesLine."Document Type"::Order,PackageLine."Source ID",PackageLine."Line No.") THEN  //jrr // RAM 05/17/2017
          //added above line as "Order Line No." is blank is package line.    //jrr 11/22/16
        // >> WC MH 05.01.09
          CLEAR(SalesLine);
        // << WC MH 05.01.09
        //<<IST 012609 CCL $12797 #12797
    ...
    begin
        ...
        IF (PackageLine.Type<>PackageLine.Type::Item) OR
            ((Package."Ship-to Type"<>Package."Ship-to Type"::Vendor) AND (Package."Ship-to Type"<>Package."Ship-to Type"::Customer)) THEN
          BEGIN
    ...
    begin
        ...
        IF NOT (LotNoInfo.GET(PackageLine."No.",PackageLine."Variant Code",PackageLine."Lot No.")) THEN
          BEGIN
            LotInfoLoaded := TRUE;
            EXIT;
          END;
    ...
    begin
        ...
        IF (UseLineNo<>0) AND (PackageLine2.GET(Package."No.",UseLineNo)) THEN
          BEGIN
    ...
    begin
        ...
        IF (UseLineNo<>0) AND (PostedPackageLine2.GET(Package."No.",UseLineNo)) THEN
          BEGIN
    ...
    begin
        ...
        IF PostedInvtPickLine.FIND('-') THEN REPEAT
    ...
    begin
        ...
        IF Package.GET(PostedInvtPickHdr."No.") THEN
          BEGIN
        ...
        //<<IST 012609 CCL $12797 #12797
            ...
            IF NOT PackageMgt.CreatePackageNIF(Package,PackingControl) THEN
              BEGIN
            ...
            REPEAT
              ...
              IF NOT PackageMgt.CreatePackageLineNIF(Package,PackingControl,PostedInvtPickLine.Quantity,Summary) THEN
                BEGIN
                  MESSAGE('%1',PackingControl."Error Message");
                  EXIT;
                END;
    ...
    begin
        ...
        IF Package.GET(SalesShptHdr."No.") THEN
          BEGIN
            IF Package.Closed THEN
              Shipping.OpenPackage(Package);
            Package.DELETE(TRUE);
          ...
          BEGIN
            ...
            REPEAT
              ...
              END ELSE BEGIN
                ...
                IF NOT PackageMgt.CreatePackageLineNIF(Package,PackingControl,SalesShptLine.Quantity,Summary) THEN
                  BEGIN
                    MESSAGE('%1',PackingControl."Error Message");
                    EXIT;
                  END;
    ...
    begin
        ...
        IF NOT PackageMgt.CreatePackageLineNIF(Package,PackingControl,PostedInvtPickLine.Quantity,FALSE) THEN
          BEGIN
            MESSAGE('%1',PackingControl."Error Message");
            EXIT;
          END;
    ...
    begin
        IF CUstNo IN ['NISSAN','RO164-CTN','RO164-MST'] THEN BEGIN
          pPath := '\\nifnavprapp\FileImport\Labels\';
          pName:= FORMAT(CallNumber)+'-'+MasterLabelNo+'-'+DELCHR(FORMAT(CURRENTDATETIME),'=','/\:')+'.txt';
            CommonOutFile.TEXTMODE(TRUE);
            CommonOutFile.WRITEMODE(TRUE);
            CommonOutFile.CREATE(pPath + pName);
            CommonOutFile.WRITE(CUstNo);
            CommonOutFile.WRITE(CartonFirstSrNo);
            CommonOutFile.WRITE(CartonLastSrNo);
            CommonOutFile.WRITE(MasterLabelNo);
            CommonOutFile.CLOSE;
        END;
    end;
}
