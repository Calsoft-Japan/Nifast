codeunit 50020 "Electronic Accounting"
{
    ...
    begin
        ...
        END ELSE ERROR(ImportFailedErr);
        ...
        IF NodeList.Count <> 0 THEN BEGIN
           ...
           IF HasAttribute(Node,'serie') AND HasAttribute(Node,'folio') THEN   FolioInvoice  := COPYSTR(Node.Attributes.GetNamedItem('serie').Value+'-'+Node.Attributes.GetNamedItem('folio').Value,1,50)
           ...
           IF HasAttribute(Node,'Moneda') THEN BEGIN
              ...
              IF HasAttribute(Node,'TipoCambio') THEN
                 IF NOT EVALUATE(CurrencyFactor,Node.Attributes.GetNamedItem('TipoCambio').Value) THEN CurrencyFactor := 1.0
              ELSE CurrencyFactor:= 1.0;
    ...
    local procedure HasAttribute(xmlElement: DotNet XmlElement;AttributeName: Text) result: Boolean
    var
    ...
    begin
        ...
        xmlNode:= xmlElement;
        FOR Indice := 0 TO xmlNode.Attributes.Count-1 DO BEGIN
    ...
    local procedure GetDateFromStr(DateStr: Text) OutDate: Date
    var
        year: Integer;
        month: Integer;
        day: Integer;
    ...
    local procedure CreatePolizaNode(var ParentNode: DotNet XmlNode;var Node: DotNet XmlNode;GLEntry: Record "17";Namespace: Text)
    begin
    ...
    begin
        ...
        IF (GLEntry."Document Type" = GLEntry."Document Type"::Payment) OR (GLEntry."Document Type" = GLEntry."Document Type"::Refund) THEN BEGIN
          IF GLEntry."Source Type" = GLEntry."Source Type"::Customer THEN BEGIN
            CASE GLEntry."Pymt - Payment Method" OF
              CodEleCheque: BEGIN
                CheckNode(Node,GLEntry,Namespace); Created:= TRUE; END;
              CodEleTransfer: BEGIN
                TransferCustomerNode(Node,GLEntry,Namespace); Created:= TRUE; END;
              ELSE  BEGIN
                OtherPaymentMethodCustNode(Node,GLEntry,Namespace); Created:= TRUE; END;
            END;
          END ELSE
            IF GLEntry."Source Type" = GLEntry."Source Type"::Vendor THEN BEGIN
              CASE GLEntry."Pymt - Payment Method" OF
                CodEleCheque: BEGIN
                  CheckNode(Node,GLEntry,Namespace); Created:= TRUE; END;
                CodEleTransfer: BEGIN
                  TransferVendorNode(Node,GLEntry,Namespace); Created:= TRUE; END;
                ELSE BEGIN
                  OtherPaymentMethodVendNode(Node,GLEntry,Namespace); Created:= TRUE;END;
              END;
            END;
        END;
    ...
    begin
        ...
        CASE GLEntry."Document Type" OF
          ...
          BEGIN
            ...
            AND (GLEntry."Source Type" = GLEntry."Source Type"::Customer) THEN BEGIN
    ...
    procedure TransferVendorNode(var ParentNode: DotNet XmlNode;GLEntry: Record "17";Namespace: Text)
    var
        CtaOri: Text[50];
        BancoOri: Code[3];
        BancoOriTag: Text[15];
    ...
    begin
        ...
        CASE GLEntry."Document Type" OF
          GLEntry."Document Type"::Payment: BEGIN
            IF Vendor.GET(GLEntry."Source No.") THEN BEGIN
              Benef := Vendor.Name;
              RFC := Vendor."VAT Registration No.";
            END;
          END;
          GLEntry."Document Type"::Refund: BEGIN
            CompanyInformation.GET;
            RFC := CompanyInformation."VAT Registration No.";
            Benef := CompanyInformation.Name;
          END;
    ...
    begin
        ...
        REPEAT
          InStream.READTEXT(TxtXML);
          XMLWellFormed:= XMLWellFormed + TxtXML;
        UNTIL InStream.EOS = TRUE;
                     MESSAGE(COPYSTR(XMLWellFormed,4330,10));
    end;
}
