codeunit 50021 "NewVision Management_New"
{
    ...
    begin
        IF UserSetup.GET(USERID) THEN BEGIN
         CASE Permission OF
          UserSetup.FIELDNO("Edit Customer"): IF UserSetup."Edit Customer" THEN EXIT(TRUE);
          UserSetup.FIELDNO("Edit Vendor"): IF UserSetup."Edit Vendor" THEN EXIT(TRUE);
          UserSetup.FIELDNO("Edit Item"): IF UserSetup."Edit Item" THEN EXIT(TRUE);
          UserSetup.FIELDNO("Gross Profit Override"): IF UserSetup."Gross Profit Override" THEN EXIT(TRUE);
          UserSetup.FIELDNO("Edit Resource"): IF UserSetup."Edit Resource" THEN EXIT(TRUE);
         END;
         EXIT(FALSE);
        END ELSE MESSAGE('You are not setup as a valid user.');
    end;
    ...
    begin
        ...
        IF GenPostingGroup.GET(GBPG,GPPG) THEN
          BEGIN
        ...
        IF Min_GPP<>0 THEN
           IF GPP < Min_GPP THEN BEGIN
            ...
            CASE GPOption OF
              GPOption::Warning: BEGIN
               MESSAGE(cString);
              END;
              GPOption::Override: BEGIN
               IF NOT CheckPermission(14017625) THEN ERROR(cString);
              END;
              GPOption::Denied: BEGIN
               ERROR(cString);
              END;
            END;
           END;
    ...
    var
        ...
        Text001: Label '%1 %2\Required Fields:\';
        RecNoRef: FieldRef;
    begin
        //>> NF1.00:CIS.CM 09-29-15
        //MessageText := '';
    ...
    begin
        ...
        CASE SourceType OF
          DATABASE::"Sales Line":
            WITH SalesLine DO
              BEGIN
                GET(SourceSubType,SourceNo,SourceLineNo);
                Item.GET("No.");
                IF Item."Item Tracking Code"='' THEN
                  EXIT;
                CALCFIELDS("E-Ship Whse. Ship. Qty (Base)","E-Ship Whse. Outst. Qty (Base)");
          ...
          DATABASE::"Transfer Line":
            WITH TransferLine DO
              BEGIN
                GET(SourceNo,SourceLineNo);
                Item.GET("Item No.");
                IF Item."Item Tracking Code"='' THEN
                  EXIT;
                CALCFIELDS("Whse. Ship. Qty (Base)","Whse. Pick Outst. Qty (Base)");
                WhseShipQtyBase := "Whse. Ship. Qty (Base)";
                WhsePickQtyBase := "Whse. Pick Outst. Qty (Base)";
        ...
        IF WhseShipQtyBase + WhsePickQtyBase <> 0 THEN
          BEGIN
            ...
            REPEAT
              IF TrackingSpecificationTmp."Quantity Handled (Base)" = 0 THEN
               ...
               ELSE
                BEGIN
                  ModifyRecord := FALSE;
                  IF TrackingSpecificationTmp."Quantity (Base)" >  TrackingSpecificationTmp."Quantity Handled (Base)" THEN
                     BEGIN
                       TrackingSpecificationTmp.VALIDATE("Quantity (Base)",TrackingSpecificationTmp."Quantity Handled (Base)");
                       ModifyRecord := TRUE;
                     END;

                  IF TrackingSpecificationTmp."Qty. to Handle" <> 0 THEN
                    BEGIN
          ...
          DATABASE::"Sales Line":
            WITH SalesLine DO
              BEGIN
                ...
                LineQtyToShip := "Qty. to Ship (Base)";
                ReserveSalesLine.InitTrackingSpecification(SalesLine,TrackingSpecification);
                ItemTrackingForm.SetSource(TrackingSpecification,SalesLine."Shipment Date");
              END;
          DATABASE::"Purchase Line":
          ...
          DATABASE::"Transfer Line":
            WITH TransferLine DO
              BEGIN
                ...
                ItemTrackingForm.SetSource(TrackingSpecification,TransferLine."Shipment Date");
              END;
           ELSE EXIT;  //shouldn't reach here, but exit just in case
        END;
        ...
        //use quantities exceeding packs only, only for MPD
          ...
          IF (QtyAvailableBase>=Item."Units per Parcel") OR
             ...
             ((Item."Units per Parcel"<>0) AND
                 ...
                 (TempLotBinContent."Location Code"<>'TENN'))) THEN
             BEGIN
        //<< NIF 06-14-05
          ...
          */
          ...
          IF LineQtyToShip >= QtyAvailableBase THEN
            BEGIN
    ...
    begin
        ...
        REPEAT
          IF NOT TempLotEntry.GET(LotEntry2."Document Type",LotEntry2."Document No.",LotEntry2."Order Line No.",0) THEN
            BEGIN
        ...
        REPEAT
          ...
          IF TempLotEntry."Document Type"=TempLotEntry."Document Type"::"Purchase Order" THEN
            BEGIN
    ...
    begin
        ...
        IF WhseItemTrkgLines.FIND('-') THEN
          REPEAT
             IF WhseItemTrkgLines."Qty. Registered (Base)"<>0 THEN
               BEGIN
    ...
    begin
        ...
        REPEAT
          ...
          IF LineQtyToPick >= QtyAvailableBase THEN
            BEGIN
              LineQty := QtyAvailableBase;
              QtyAvailableBase := 0;
    ...
    begin
        ...
        IF PAGE.RUNMODAL(0,CustItemDrawing)=ACTION::LookupOK THEN
          BEGIN
    ...
    begin
        WITH LotBinContent DO
          BEGIN
            IF LotBinContentAlloc.GET("Location Code","Bin Code","Item No.","Variant Code","Unit of Measure Code","Lot No.") THEN BEGIN
        ...
        IF Location.FIND('-') THEN
          REPEAT
            ...
            WITH LotNoInfo DO BEGIN
             IF FIND('-') THEN
              REPEAT
               IF NOT TempLotBinContent.GET(Location.Code,'',"Item No.","Variant Code",Item."Base Unit of Measure","Lot No.") THEN BEGIN
                 TempLotBinContent.INIT;
                 TempLotBinContent."Location Code" := Location.Code;
                 TempLotBinContent."Item No." := "Item No.";
                 TempLotBinContent."Bin Code" := '';
                 TempLotBinContent."Variant Code" := "Variant Code";
                 TempLotBinContent."Unit of Measure Code" := Item."Base Unit of Measure";
                 TempLotBinContent."Lot No." := "Lot No.";
                 TempLotBinContent."Qty. per Unit of Measure" := 1;
                 IF InsertQty THEN BEGIN
                   LotNoInfo.CALCFIELDS(Inventory);
                   TempLotBinContent."Qty. to Handle" := Inventory;
                   TempLotBinContent."Qty. to Handle (Base)" := Inventory;
                 END;
                 TempLotBinContent.INSERT;
               END;
              UNTIL NEXT=0;
            END;  //END WITH LotNoInfo DO
    ...
    begin
        ...
        IF NOT ToLotNoInfo.GET(Item."No.",'',ToLotNo) THEN
          BEGIN
            ToLotNoInfo.INIT;
        ...
        IF NOT FromLocation.GET(FromLocCode) THEN
         BEGIN
           ErrorMsg := 'From Loc not found';
           EXIT(FALSE);
         END;

        IF NOT ToLocation.GET(ToLocCode) THEN
         BEGIN
        ...
        //valid to to bin
        IF (NOT ToBin.GET(ToLocCode,ToBinCode)) AND (ToLocation."Bin Mandatory") THEN
         BEGIN
        ...
        //valid from bin
        IF (NOT FromBin.GET(FromLocCode,FromBinCode)) AND (FromLocation."Bin Mandatory") THEN
          BEGIN
        ...
        IF AppliesToEntryNoGbl <> 0 THEN BEGIN
          ...
          IF TempILEDimSetEntry.FINDSET THEN BEGIN
            REPEAT
        ...
        IF (ItemTrackingCode.GET(Item."Item Tracking Code")) AND (ItemTrackingCode."Lot Pos. Adjmt. Outb. Tracking") THEN
          BEGIN
            ...
            WITH ReservEntry DO
              BEGIN
                INIT;
                "Entry No.":= UseEntryNo;
                Positive:= (ItemJnlLine.Quantity>0);
                ...
                "Creation Date":= ItemJnlLine."Posting Date";
                "Source Type":= 83;
                "Source Subtype" := 4;
                ...
                "Shipment Date" := ItemJnlLine."Posting Date";
                "Created By":= USERID;
                ...
                "Qty. to Handle (Base)":= -ItemJnlLine."Quantity (Base)";
                "Qty. to Invoice (Base)":= -ItemJnlLine."Quantity (Base)";
                "Mfg. Lot No." := MfgLotNo;
                "New Lot No." := ToLotNo;
                "Lot No.":= FromLotNo;
                //NIF1.01 Start
                IF AppliesToEntryNoGbl <> 0 THEN
                  VALIDATE("Appl.-to Item Entry",AppliesToEntryNoGbl);
                //NIF1.01 End
                INSERT;
              END;
    ...
    begin
        ...
        IF NOT Location.GET(FromLocCode) THEN
         BEGIN
           ErrorMsg := 'From Loc not found';
           EXIT(FALSE);
         END;
        
        IF NOT Location.GET(ToLocCode) THEN
         BEGIN
           ErrorMsg := 'To Loc not found';
           EXIT(FALSE);
         END;
        ...
        IF ItemType = ItemType::"Lot No." THEN
          BEGIN
            //check item
            IF NOT Item.GET(LotNoInfo."Item No.") THEN
              BEGIN
        ...
        //valid to to bin
        IF NOT ToBin.GET(ToLocCode,ToBinCode) THEN
         BEGIN
           ErrorMsg := COPYSTR(STRSUBSTNO('To Bin %1 invalid',ToBinCode),1,30);
           EXIT(FALSE);
          END;
        ...
        //valid from bin
        IF NOT FromBin.GET(FromLocCode,FromBinCode) THEN
          BEGIN
           ErrorMsg := COPYSTR(STRSUBSTNO('From Bin %1 invalid',FromBinCode),1,30);
           EXIT(FALSE);
          END;
    ...
    begin
        ...
        IF LotNo='' THEN
          BEGIN
            ...
            IF Item."Item Tracking Code"='' THEN
              ItemType := ItemType::"Item No."
            //else if invalid item tracking code, return error
            ELSE IF NOT ItemTrackingCode.GET(Item."Item Tracking Code") THEN
              BEGIN
                ErrorMsg := COPYSTR(STRSUBSTNO('Invalid Trkg. Code: %1',Item."No."),1,30);
                EXIT(FALSE);
              END
            //otherwise, return error if item is lot tracked
            ELSE IF ItemTrackingCode."Lot Specific Tracking" THEN
              BEGIN
                ErrorMsg := COPYSTR(STRSUBSTNO('Lot mandatory: Item %1.',Item."No."),1,30);
                EXIT(FALSE);
              END;
        ...
        //valid to to bin
        IF NOT ToBin.GET(FromLocCode,ToBinCode) THEN
         BEGIN
        ...
        WhseJnlLine."Posting Date" := TODAY;
    ...
    begin
        ...
        WITH DimSetEntry_vRec DO BEGIN
          IF DimensionValue_iCod <> '' THEN BEGIN
            RESET;
            SETRANGE("Dimension Code",DimensionCode_iCod);
            IF NOT FINDFIRST THEN BEGIN
              INIT;
              VALIDATE("Dimension Code",DimensionCode_iCod);
              VALIDATE("Dimension Value Code",DimensionValue_iCod);
              INSERT(TRUE);
              EXIT(TRUE);
            END ELSE BEGIN
              IF "Dimension Value Code" <> DimensionValue_iCod THEN BEGIN;
                VALIDATE("Dimension Value Code",DimensionValue_iCod);
                MODIFY(TRUE);
                EXIT(TRUE);
              END;
