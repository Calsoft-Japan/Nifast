codeunit 50133 "FB Management"
{
    ...
    begin
        ...
        CASE FBImportLog."Quantity Type" OF
            FBImportLog."Quantity Type"::Count:
    ...
    begin
        ...
        //<< NIF 01-12-06 #10566
        IF FBHeader.FIND('-') THEN
            REPEAT
                CASE FBHeader."FB Order Type" OF
                    FBHeader."FB Order Type"::" ":
                        BEGIN
                            FBLine.RESET();
                            ...
                            FBHeader.MODIFY();
                        END;
                    FBHeader."FB Order Type"::Consigned:
                        BEGIN
                            FBLine.RESET();
                            FBLine.SETRANGE("Document No.", FBHeader."No.");
                            IF NOT FBLine.ISEMPTY THEN
                            ...
                            FBHeader.MODIFY();
                        END;
                    FBHeader."FB Order Type"::"Non-Consigned":
                        BEGIN
                            FBLine.RESET();
                            FBLine.SETRANGE("Document No.", FBHeader."No.");
                            FBLine.SETRANGE("Replenishment Method", FBLine."Replenishment Method"::Automatic);
    ...
    begin
        ...
        // SalesHeader.VALIDATE("Contract No.", FBHeader."Contract No.");//TODO
        //>> NIF 12-08-05
        ...
        IF FBLine.FIND('-') THEN
            REPEAT
                ...
                // SalesLine.VALIDATE("Contract No.", FBLine."Contract No.");//TODO
    ...
    begin
        ...
        //TransferHeader."FB Order No." := FBHeader."No.";//TODO
    ...
    var
        SalesPrice: Record 7002;
        FBLine: Record 50137;
        TEXT001: Label 'Item No. is blank for Line %1. Do you want to continue?', Comment = '%1';
        Text002: Label 'The %1 for Item %2 UOM %3 does not match Contract. Do you want to continue?', Comment = '%1 %2 %3';
        text003: Label 'Item %1 UOM %2 not found on Contract %3. Do you want to continue?', Comment = '%1 %2 %3';
    begin
        FBLine.SETRANGE("Document No.", FBHeader."No.");
        IF FBLine.FIND('-') THEN
            REPEAT
                ...
                //>>NIF 031306 RTT
                IF FBLine."Item No." = '' THEN BEGIN
                    IF NOT CONFIRM(STRSUBSTNO(TEXT001, FBLine."Line No.")) THEN
    ...
    begin
        ...
        SalesPrice.SETRANGE("Sales Type", SalesPrice."Sales Type"::Customer);
        SalesPrice.SETRANGE("Sales Code", FBLine."Sell-to Customer No.");
        // SalesPrice.SETRANGE("Method of Fullfillment", SalesPrice."Method of Fullfillment"::FillBill);  //NF1.00:CIS.NG 10/26/15//TODO
        IF FBLine."Contract No." <> '' THEN
            SalesPrice.SETRANGE("Contract No.", FBLine."Contract No.");
