report 50003 "Aged Accounts Payable New"
{
    ...
    dataset
    {
        dataitem(Vendor; Vendor)
        {
                ...
                begin
                    ...
                    END ELSE BEGIN
                        IF ShowARC THEN BEGIN
                            TempVendLedgEntry.CALCFIELDS("Remaining Amt. (LCY)");
                            //MESSAGE('1-%1\%2',TempVendLedgEntry."Remaining Amount",TempVendLedgEntry."Remaining Amt. (LCY)");
                            IF TempVendLedgEntry."Currency Code" = GLSetup."Additional Reporting Currency" THEN
                                AmountDueToPrint := TempVendLedgEntry."Remaining Amount"
                            ELSE IF TempVendLedgEntry."Currency Code" = GLSetup."LCY Code" THEN
                                AmountDueToPrint := ExchangeAmtLCYToFCY(PeriodEndingDate[1], TempVendLedgEntry."Remaining Amount", FALSE)
                            ELSE
                                AmountDueToPrint := ExchangeAmtLCYToFCY(PeriodEndingDate[1], TempVendLedgEntry."Remaining Amt. (LCY)", FALSE);
                        END;
                    END;
                    //SM001 1/26/18
                    //IF ShowARC THEN
                    ...
                    //>> NF1.00:CIS.CM 08-13-15
                    TotalAmountDueToPrint_gDec += AmountDueToPrint;
                    GrandTotalAmountToPrint_gDec += AmountDueToPrint;
                    //<< NF1.00:CIS.CM 08-13-15
                    ...
                    //>> NF1.00:CIS.CM 08-13-15
                    TotalAmountDue_gDecArr[j] += AmountDue[j];
                    GrandTotalAmountDue_gDecArr[j] += AmountDue[j];
                    //<< NF1.00:CIS.CM 08-13-15
                    ...
                    PrevVendorNo_gCod := TempVendLedgEntry."Vendor No.";  //NF1.00:CIS.CM 08-13-15
                end;
                ...
                begin
                    IF TempVendLedgEntry.COUNT > 0 THEN BEGIN
                        ...
                        IF NOT PrintDetail AND PrintToExcel THEN
                            MakeExcelDataBody;
                        //>> NF1.00:CIS.CM 08-13-15
                        PrevVendor_gRec.GET(PrevVendorNo_gCod);
                        MakeExcelDataGroupFooter_lFnc;
                        CLEAR(PrevVendor_gRec);
                        CLEAR(TotalAmountDueToPrint_gDec);
                        CLEAR(TotalAmountDue_gDecArr);
                        //<< NF1.00:CIS.CM 08-13-15
                    END;
                end;
            ...
            begin
                //>> NF1.00:CIS.CM 08-13-15
                CalcPercents(GrandTotalAmountToPrint_gDec, GrandTotalAmountDue_gDecArr);
                IF PrintToExcel THEN
                    MakeExcelDataFooter_lFnc;
                //<< NF1.00:CIS.CM 08-13-15
            end;
    ...
    var
        ...
        PrevVendorNo_gCod: Code[20];
        PrevVendor_gRec: Record 23;
        TotalAmountDueToPrint_gDec: Decimal;
        TotalAmountDue_gDecArr: array[4] of Decimal;
        GrandTotalAmountToPrint_gDec: Decimal;
        GrandTotalAmountDue_gDecArr: array[4] of Decimal;
        Text50000_gCtx: Label 'Report Total Amount Due($)';
        ShowARC: Boolean;
        ShowARCcaption: Text;
    ...
    local procedure MakeExcelDataGroupFooter_lFnc()
    begin
        //>> NF1.00:CIS.CM 08-13-15
        ExcelBuf.NewRow;
        ExcelBuf.AddColumn(PrevVendor_gRec."No.", FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn(PrevVendor_gRec.Name, FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        IF PrintDetail THEN BEGIN
            ExcelBuf.AddColumn(Total_Amount_DueCaptionLbl, FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
            IF PrintAmountsInLocal THEN
                ExcelBuf.AddColumn(GLSetup."LCY Code", FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text)
            ELSE
                ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
            ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
            ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        END;
        ExcelBuf.AddColumn(-TotalAmountDueToPrint_gDec, FALSE, '', TRUE, FALSE, FALSE, '#,##0.00', ExcelBuf."Cell Type"::Number);
        ExcelBuf.AddColumn(-TotalAmountDue_gDecArr[1], FALSE, '', TRUE, FALSE, FALSE, '#,##0.00', ExcelBuf."Cell Type"::Number);
        ExcelBuf.AddColumn(-TotalAmountDue_gDecArr[2], FALSE, '', TRUE, FALSE, FALSE, '#,##0.00', ExcelBuf."Cell Type"::Number);
        ExcelBuf.AddColumn(-TotalAmountDue_gDecArr[3], FALSE, '', TRUE, FALSE, FALSE, '#,##0.00', ExcelBuf."Cell Type"::Number);
        ExcelBuf.AddColumn(-TotalAmountDue_gDecArr[4], FALSE, '', TRUE, FALSE, FALSE, '#,##0.00', ExcelBuf."Cell Type"::Number);
        ExcelBuf.NewRow;
        ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        IF PrintDetail THEN BEGIN
        ...
        END;
        ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn(FORMAT(PercentString[1]), FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn(FORMAT(PercentString[2]), FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn(FORMAT(PercentString[3]), FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn(FORMAT(PercentString[4]), FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.NewRow;
        //<< NF1.00:CIS.CM 08-13-15
    end;

    local procedure MakeExcelDataFooter_lFnc()
    begin
        //>> NF1.00:CIS.CM 08-13-15
        ExcelBuf.NewRow;
        ExcelBuf.AddColumn(FORMAT(Text50000_gCtx), FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        IF PrintDetail THEN BEGIN
            ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
            ExcelBuf.AddColumn('', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
