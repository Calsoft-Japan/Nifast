xmlport 50028 "Import PPS File1"
{
        ...
        textelement(Root)
        {
            ...
            tableelement(Integer; Integer)
            {
                ...
                begin
                    ...
                    IF NOT PPSShipTo.FIND('-') THEN BEGIN
                        ErrorFlag := TRUE;
                        ErrorMessage := STRSUBSTNO(Text00Lbl, ZoneCode);
                        PPSBuffer.INIT();
            ...
            tableelement("<integerd>"; Integer)
            {
                ...
                begin
                    ...
                    ItemCrossRef.SETFILTER("Reference Type No.", '%1|%2', '1', 'CAMI');
                    IF NOT ItemCrossRef.FIND('-') THEN BEGIN
                        CrossRefFound := FALSE;
                        ErrorFlag := TRUE;
                        ErrorMessage := STRSUBSTNO(TextLbl, PartNo);
                        MESSAGE('Error in File %1:\' +
                                '%2', UseFileName2, ErrorMessage);
                    END ELSE
                        CrossRefFound := TRUE;
                    ...
                    IF CrossRefFound THEN BEGIN
                        ...
                        IF Item."Units per Parcel" = 0 THEN BEGIN
                            ErrorFlag := TRUE;
                            ErrorMessage := STRSUBSTNO(Text03Lbl, Item."No.", PartNo);
                            MESSAGE('Error in File %1:\' +
                                  '%2', UseFileName2, ErrorMessage);
                        END;
                        //Tax Group Code
                        IF Item."Tax Group Code" = '' THEN BEGIN
                            ErrorFlag := TRUE;
                            ErrorMessage := STRSUBSTNO(Text02Lbl, Item."No.");
                            MESSAGE('Error in File %1:\' +
                                  '%2', UseFileName2, ErrorMessage);
                        END;
                    END;
    ...
    var
        ...
        TextLbl: Label 'No Cross Reference found for Part Number %1.', Comment = '%1';
        Text00Lbl: Label 'No Ship-to Code found in Ship-to PPS file for Zone %1.', Comment = '%1';
        Text01Lbl: Label 'No Price Contract found for Item %1, Cross Ref No. %2.', Comment = '%1%2';
        Text02Lbl: Label 'Tax Group Code for Item %1 is blank.', Comment = '%1';
        Text03Lbl: Label 'No SNP found for Item %1, Cross Ref No. %2.', Comment = '%1%2';
